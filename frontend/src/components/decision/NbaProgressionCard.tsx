...
export const NbaProgressionCard: React.FC<NbaProgressionCardProps> = ({
  ...
  // Reset confirmation state when switching cases
  useEffect(() => {
  ...
  const pendingGatedCount = actions.filter(
    a => (a.approval_tier === 'L1' || a.approval_tier === 'L2') && 
         (a.state === 'pending_approval' || a.state === 'recommended')
  ).length;
  ...
  const isResolved = (currentCase.status === 'resolved_fraud' || currentCase.status === 'resolved_cleared') && !hasPendingApproval;
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-4 select-none text-slate-800 transition-all duration-200">
      ...
      <div>
        ...
        {/* Lightweight indicator when closure is actively blocked by pending gated actions */}
        {pendingGatedCount > 0 && !isPreInvestigation && (
          <div className="mb-2.5 flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-amber-50/90 border border-amber-200/90 text-amber-900 text-[11px] leading-tight shadow-2xs">
        ...
        {isPreInvestigation ? (
          ...
          <div className="space-y-2">
            {actions.map((action, idx) => {
              ...
              const isL2 = action.approval_tier === 'L2';
              ...
              if (isPending && isL1L2) {
                return (
                  <div
                    key={action.id}
                    className={`p-2.5 rounded-lg border shadow-2xs transition-all space-y-2 ${
                      isL2
                  ...
                  >
                      ...
                      <div className="flex items-start gap-2.5 min-w-0 flex-1">
                        ...
                        <div 
                          className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5 ${
                            isL2 
                              ? 'bg-purple-900 text-white border border-purple-950 shadow-xs' 
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            <span 
                              className={`text-[9.5px] font-mono font-bold px-1.5 py-0.2 rounded border ${
                                isL2 
                                  ? 'bg-purple-900 text-purple-100 border-purple-950 shadow-xs' 
                    ...
                    {/* Inline Confirmation for L2 or Standard Approve & Deny Button Pair */}
                    {confirmingActionId === action.id ? (
                      <div className="pt-2 border-t border-purple-200/80 mt-1">
                        <div className="p-2.5 rounded-lg bg-purple-950/5 border border-purple-300 text-purple-950 animate-in fade-in duration-150">
                          <div className="flex items-center gap-1.5 text-[11.5px] font-semibold text-purple-950">
                            <ShieldAlert className="w-3.5 h-3.5 text-purple-700 shrink-0" />
                            <span>Confirm senior sign-off for {action.title}?</span>
                          </div>
                          <div className="mt-2.5 flex items-center justify-end gap-2">
                            <button
                              ...
                              className="px-2.5 py-1 rounded text-[11px] font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 border border-slate-200 transition-colors cursor-pointer"
                            >
                              Cancel
                            </button>
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setConfirmingActionId(null);
                                onApproveAction?.(currentCase.case_id, action.id);
                              }}
                              className="px-2.5 py-1 rounded bg-purple-800 hover:bg-purple-900 text-white font-semibold text-[11px] flex items-center gap-1 shadow-xs transition-colors cursor-pointer"
                            >
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-slate-200/80">
                        <span className="text-[10px] text-slate-500 italic flex items-center gap-1">
                          <span>
                            {isL2 ? 'Awaiting senior sign-off' : 'Awaiting analyst sign-off'}
                          </span>
                        </span>
